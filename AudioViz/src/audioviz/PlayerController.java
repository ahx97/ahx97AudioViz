...
public class PlayerController implements Initializable {
    ...
    private Duration durationOfSong;
    ...
    private void handleReady() {
        ...
        durationOfSong = mediaPlayer.getTotalDuration();
        Double durationDouble = duration.toMillis();
        DecimalFormat df = new DecimalFormat("#.0");
        String durationString = df.format(durationDouble);
        lengthText.setText(durationString);
    ...
    private void handleUpdate(double timestamp, double duration, float[] magnitudes, float[] phases) {
        ...
        DecimalFormat df = new DecimalFormat("#.0");
        String timeString = df.format(ms);
        currentText.setText(timeString);
    ...
    @FXML
    private void handlePickUPSlider(ActionEvent event){
        mediaPlayer.pause();
    }
    
    @FXML
    private void handleClicked(Event event){
        mediaPlayer.pause();
        Duration duration = mediaPlayer.getTotalDuration();
        Double durationDouble = duration.toMillis();//current time
        double ms = timeSlider.getValue();//current slider value
        System.out.println("doubleDuration: " + durationDouble);
        System.out.println("ms: " + ms);
        mediaPlayer.seek(duration.multiply(timeSlider.getValue()/durationDouble));
        mediaPlayer.play();
    }
